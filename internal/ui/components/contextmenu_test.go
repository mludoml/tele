...
func TestContextMenu_HasForwardItem(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, 0, false, false, 0, false, nil, defaultKM())
	assert.Contains(t, strip(cm.View()), "forward")
}

func TestContextMenu_F_EmitsForwardRequest(t *testing.T) {
	cm := components.NewContextMenu(7, false, 0, 0, 0, false, false, 0, false, nil, defaultKM())
...
func TestNewContextMenu_OutgoingItems(t *testing.T) {
	cm := components.NewContextMenu(1, true, 0, 0, 0, false, false, 0, false, nil, defaultKM())
	view := strip(cm.View())
	assert.Contains(t, view, "reply")
	assert.Contains(t, view, "React")
...
func TestContextMenu_EscFromMain_Closes(t *testing.T) {
	cm := components.NewContextMenu(42, false, 0, 0, 0, false, false, 0, false, nil, defaultKM())
...
func TestContextMenu_Edit_EmitsEditMsgRequest(t *testing.T) {
	cm := components.NewContextMenu(42, true, 0, 0, 0, false, false, 0, false, nil, defaultKM())
...
func TestContextMenu_Reply_OutgoingMessage_EmitsReplyMsgRequest(t *testing.T) {
	cm := components.NewContextMenu(99, true, 0, 0, 0, false, false, 0, false, nil, defaultKM())
...
func TestNewContextMenu_IsReply_ShowsJumpToOriginal(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 42, 0, false, false, 0, false, nil, defaultKM())
	view := strip(cm.View())
...
func navigateToIncomingDeleteSubPrompt(t *testing.T) *components.ContextMenu {
	t.Helper()
	cm := components.NewContextMenu(77, false, 0, 0, 0, false, false, 0, false, nil, defaultKM())
	// incoming: Reply(0), React(1), Forward(2), Delete(3)
	cm, _ = cm.Update(pressJ())
	require.NotNil(t, cm)
	cm, _ = cm.Update(pressJ())
...
func TestNewContextMenu_PhotoMessage_ShowsAllThreeActions(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaPhoto, true, false, 0, false, photoTargets(), defaultKM())
	view := strip(cm.View())
...
func TestNewContextMenu_VideoMessage_ShowsAllThreeActions(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaVideo, true, false, 0, false, videoTargets(), defaultKM())
	view := strip(cm.View())
...
func TestContextMenu_Photo_OpenExternal_EmitsOpenExternalRequest(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaPhoto, true, false, 0, false, nil, defaultKM())
...
func TestContextMenu_Video_OpenInApp_EmitsOpenInViewerRequest(t *testing.T) {
	cm := components.NewContextMenu(42, false, 0, 0, domain.MediaVideo, true, false, 0, false, videoTargets(), defaultKM())
...
func TestContextMenu_Photo_OpenInApp_EmitsOpenInViewerRequest(t *testing.T) {
	cm := components.NewContextMenu(7, false, 0, 0, domain.MediaPhoto, true, false, 0, false, photoTargets(), defaultKM())
	newCM, cmd := cm.Update(pressO())
	assert.Nil(t, newCM)
	require.NotNil(t, cmd)
	_, ok := cmd().(components.OpenInViewerRequest)
...
func TestNewContextMenu_VoiceMessage_ShowsPlayAndDownload(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaVoice, true, false, 0, false, nil, defaultKM())
	view := strip(cm.View())
...
func TestNewContextMenu_GIFMessage_ShowsDownloadOnly(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaGIF, true, false, 0, false, nil, defaultKM())
	view := strip(cm.View())
...
func TestNewContextMenu_StickerMessage_ShowsNoMediaActions(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaSticker, true, false, 0, false, nil, defaultKM())
	view := strip(cm.View())
	assert.NotContains(t, view, "Download")
...
func TestNewContextMenu_HasText_ShowsCopy(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, 0, false, true, 0, false, nil, defaultKM())
...
func TestContextMenu_Copy_EmitsCopyMsgRequest(t *testing.T) {
	cm := components.NewContextMenu(42, false, 0, 0, 0, false, true, 0, false, nil, defaultKM())
...
func TestNewContextMenu_SingleLink_ShowsOpenLink(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, 0, false, true, 0, false, linkTarget(), defaultKM())
...
func TestContextMenu_Link_Open_EmitsOpenInViewerRequest(t *testing.T) {
	cm := components.NewContextMenu(5, false, 0, 0, 0, false, true, 0, false, linkTarget(), defaultKM())
	newCM, cmd := cm.Update(pressO())
	assert.Nil(t, newCM)
	require.NotNil(t, cmd)
	_, ok := cmd().(components.OpenInViewerRequest)
...
func TestNewContextMenu_FileShowsDownload(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaFile, true, false, 0, false, nil, defaultKM())
	assert.Contains(t, strip(cm.View()), "save file (download)")
}

func TestContextMenu_Download_EmitsDownloadFileRequest(t *testing.T) {
	cm := components.NewContextMenu(42, false, 0, 0, domain.MediaFile, true, false, 0, false, nil, defaultKM())
...
func TestContextMenu_TextMessage_OffersTranslate(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, 0, false, true, 1, false, nil, defaultKM())
	view := strip(cm.View())
	assert.Contains(t, view, "Translate")
	assert.NotContains(t, view, "Show original")
}

func TestContextMenu_TranslatedMessage_OffersShowOriginal(t *testing.T) {
	cm := components.NewContextMenu(1, false, 0, 0, 0, false, true, 1, true, nil, defaultKM())
	view := strip(cm.View())
	assert.Contains(t, view, "Show original")
	assert.NotContains(t, view, "Translate")
}

func TestContextMenu_NoTranslationTarget_HidesTranslateRow(t *testing.T) {
	// translationMsgID 0 is a textless message: nothing to translate, so the row
	// is absent even though the message carries media actions.
	cm := components.NewContextMenu(1, false, 0, 0, domain.MediaPhoto, true, false, 0, false, photoTargets(), defaultKM())
	view := strip(cm.View())
	assert.NotContains(t, view, "Translate")
	assert.NotContains(t, view, "Show original")
...
func TestContextMenu_TranslateEnables(t *testing.T) {
	// incoming text message: Reply(0) React(1) Forward(2) Copy text(3) Translate(4)
	cm := components.NewContextMenu(42, false, 0, 0, 0, false, true, 42, false, nil, defaultKM())
	for range 4 {
		cm, _ = cm.Update(pressJ())
		require.NotNil(t, cm)
	}
	...
	assert.Nil(t, newCM)
	require.NotNil(t, cmd)
	req, ok := cmd().(components.TranslateMsgRequest)
	require.True(t, ok)
	assert.Equal(t, 42, req.MsgID)
	assert.True(t, req.Enable)
}

func TestContextMenu_ShowOriginalDisables(t *testing.T) {
	// Same rows, but the message is already shown translated.
	cm := components.NewContextMenu(42, false, 0, 0, 0, false, true, 42, true, nil, defaultKM())
	for range 4 {
		cm, _ = cm.Update(pressJ())
		require.NotNil(t, cm)
	}
	...
	require.True(t, ok)
	assert.Equal(t, 42, req.MsgID)
	assert.False(t, req.Enable, "Show original asks for the original back")
}

// A collapsed album's caption lives on one part, not on the anchor. Here the
// selection anchors on 100 while the caption-bearing part is 101: Reply and Edit
// must keep addressing 100, and only Translate follows the caption.
func TestContextMenu_TranslateAddressesCaptionPartNotAnchor(t *testing.T) {
	// outgoing photo album with caption: Reply(0) React(1) Forward(2) Copy text(3)
	// Translate(4) Open photo(5) Edit(6) …
	cm := components.NewContextMenu(100, true, 0, 0, domain.MediaPhoto, true, true, 101, false, photoTargets(), defaultKM())

	next, cmd := cm.Update(pressEnter())
	assert.Nil(t, next)
	require.NotNil(t, cmd)
	reply, ok := cmd().(components.ReplyMsgRequest)
	require.True(t, ok)
	assert.Equal(t, 100, reply.MsgID, "Reply keeps the album anchor")

	cm, _ = cm.Update(pressJ()) // React
	cm, _ = cm.Update(pressJ()) // Forward
	cm, _ = cm.Update(pressJ()) // Copy text
	cm, _ = cm.Update(pressJ()) // Translate
	require.NotNil(t, cm)
	next, cmd = cm.Update(pressEnter())
	assert.Nil(t, next)
	require.NotNil(t, cmd)
	tr, ok := cmd().(components.TranslateMsgRequest)
	require.True(t, ok)
	assert.Equal(t, 101, tr.MsgID, "Translate addresses the caption-bearing part")
	assert.True(t, tr.Enable)

	cm2 := components.NewContextMenu(100, true, 0, 0, domain.MediaPhoto, true, true, 101, false, photoTargets(), defaultKM())
	next, cmd = cm2.Update(keyMsg('e'))
	assert.Nil(t, next)
	require.NotNil(t, cmd)
	edit, ok := cmd().(components.EditMsgRequest)
	require.True(t, ok)
	assert.Equal(t, 100, edit.MsgID, "Edit keeps the album anchor")
}

func TestContextMenu_TranslateRowSurvivesDeleteSubMenu(t *testing.T) {
	cm := components.NewContextMenu(100, false, 0, 0, 0, false, true, 101, false, nil, defaultKM())
	cm, _ = cm.Update(pressD()) // delete sub-menu
	require.NotNil(t, cm)
	cm, _ = cm.Update(pressEsc()) // back to main
	require.NotNil(t, cm)
	require.Equal(t, 0, cm.Cursor())
	for range 4 {
		cm, _ = cm.Update(pressJ())
		require.NotNil(t, cm)
	}
	...
	req, ok := cmd().(components.TranslateMsgRequest)
	require.True(t, ok)
	assert.Equal(t, 101, req.MsgID, "the row keeps its target after the round trip")
}

func TestOutboxContextMenu_HasNoTranslateRow(t *testing.T) {
	cm := components.NewOutboxContextMenu("ref-1", true, defaultKM())
	view := strip(cm.View())
	assert.Contains(t, view, "Retry send")
	assert.NotContains(t, view, "Translate")
	assert.NotContains(t, view, "Show original")
}
